...
namespace avmshell
{
	...
	int Shell::execute(int argc, char *argv[])
	{
		...
		TRY(this, kCatchAction_ReportAsError)
		{
			...
			struct App {
				//TODO check remove toplevel - constant
				Toplevel* toplevel;
				Domain* domain;
				DomainEnv* domainEnv;
				ShellCodeContext* codeContext;
				Stringp scriptName;
				int mTime;//last modified time of abc file
			};

			Hashtable* apps = new (GetGC()) Hashtable(GetGC());



			const char* fname;

			while ((fname = getRequest()))
			{
				HTTPClass::outputStarted = false;

				Stringp strFname = internStringUTF8(fname);
				Atom res = apps->get(strFname->atom());

				App* app;


				//check file las modified
				struct ::stat st;
				//TODO check if stat failed
				::stat(fname, &st);
				int mTime = (int) st.st_mtime;

				if (res != undefinedAtom)
				{
					app = (App*) res;
					if (app->mTime != mTime)
					{
						apps->remove(strFname->atom());
						delete app;
						res = undefinedAtom;
				...
				//init sapi
				modas::Sapi sapi(console, httpRequest);
				HTTPFormClass::sapi = &sapi;

				if (res == undefinedAtom)
				{
					console << "Loading app " << strFname << "\n";

					//this is a new request, we load the app
					app = new App;
					app->mTime = mTime;
					app->toplevel = initShellBuiltins();
					app->domain = new (GetGC()) Domain(this, builtinDomain);
					app->domainEnv = new (GetGC()) DomainEnv(this, app->domain, app->toplevel->domainEnv());
					app->codeContext = new (GetGC()) ShellCodeContext();
					app->codeContext->m_domainEnv = app->domainEnv;

					apps->put(strFname->atom(), app);

					//load abc file
					FileInputStream f(fname);
					...
					if (isValid)
					{
						...
						handleActionBlock(code, 0, app->domainEnv, app->toplevel, NULL, app->codeContext);
						apps->put(strFname->atom(), app);
					}
					else
					{
						delete app;//TODO needs destructor?
						console << "cannot open file: " << fname << "\n";
						HTTPClass::sWrite("Status: 404\n\nSource not found");
						//return 1;
						continue;
					}
				}

				// BEGIN execute method Main.main in loaded code
				Multiname mn(publicNamespace, internString(newConstantStringLatin1("Main")));
				ScriptEnv* script = (ScriptEnv*) app->domainEnv->getScriptInit(mn);
				if (script == NULL)
					console << "No Main class found! You need to define a class Main with a static main()\n\n";
				else
				{
					...
					Atom atom = app->toplevel->getproperty(script->global->atom(),
							&mn, script->global->vtable);
					ClassClosure* cc = (ClassClosure*)AvmCore::atomToScriptObject(atom);
					//console << atomToString(cc->toString());

					Stringp fnName = internString(newConstantStringLatin1("main"));
					Binding binding = cc->traits()->getTraitsBindings()->findBinding(fnName);
					MethodEnv *method = cc->vtable->methods[AvmCore::bindingToMethodId(binding)];

					// and construct the 'this' arg
					Atom args[1] = { cc->atom() };
					method->coerceEnter(0, args);
				}
				// END execute method Main.main in loaded code

				//BEGIN execute __Cleanup.clean()
				{
					Multiname mn(publicNamespace, internString(newConstantStringLatin1("__Cleanup")));
					ScriptEnv* script = (ScriptEnv*) app->domainEnv->getScriptInit(mn);
					Atom atom = app->toplevel->getproperty(script->global->atom(),
							&mn, script->global->vtable);
					ClassClosure* cc = (ClassClosure*)AvmCore::atomToScriptObject(atom);
					...
					Stringp fnName = internString(newConstantStringLatin1("clean"));
					Binding binding = cc->traits()->getTraitsBindings()->findBinding(fnName);
					MethodEnv *method = cc->vtable->methods[AvmCore::bindingToMethodId(binding)];
				...
				//END execute Upload.cleanup()


				if (!HTTPClass::outputStarted)
					HTTPClass::sWrite("Content-type: text/html\r\n\r\n");
		...
		CATCH(Exception *exception)
		{
			...
			if (!(exception->flags & Exception::SEEN_BY_DEBUGGER))
			{
				...
				HTTPClass::sWrite("<div style=\"color:red\">ActionScript error.<br/>");
				StUTF8String str(string(exception->atom));
				HTTPClass::sWrite(str.c_str());
				HTTPClass::sWrite("</div>");
			}
			if (exception->getStackTrace()) {
				console << exception->getStackTrace()->format(this) << '\n';

				HTTPClass::sWrite("<pre style=\"color:red\">");
				StUTF8String str(exception->getStackTrace()->format(this));
				HTTPClass::sWrite(str.c_str());
				HTTPClass::sWrite("</pre>");
			}
			#else
			// [ed] always show error, even in release mode,
			// see bug #121382
	...
	/* static */
	int Shell::run(int, char *argv[])
	{
		MMgc::GCHeapConfig conf;
		//conf.verbose = true;
		MMgc::GCHeap::Init(conf);

		//run starter abc, same filename + .abc
		char* str = new char[strlen(argv[0])+5];
		strcpy(str, argv[0]);
		strcat(str, ".abc");
		char* argv2[] = {argv[0], str/*, "-memstats"*/};
		...
		int exitCode = 0;
		{
			...
			if(MMGC_ENTER_STATUS == 0)
			{
				...
				exitCode = shell->execute(2, argv2);
				//HTTPFormClass::reset();
				delete shell;
				delete gc;
				delete[] str;
